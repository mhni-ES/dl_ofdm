...
def load_model(path, session):
    graph = session.graph
    saver = tf.compat.v1.train.import_meta_graph(path + '.meta', clear_devices=True)
...
def dense_block_tx(inputs, FLAGS):
    ...
    # Traditional Coding
    code1 = tf.compat.v1.layers.dense(code0,
                          codeblockin,
    ...
    out = tf.compat.v1.layers.dense(out,
                          codeblockout,
                          kernel_regularizer=tf.keras.regularizers.l2(l=0.01),
                          bias_regularizer=tf.keras.regularizers.l2(l=0.01),
...
def conv_block_tx(inputs, FLAGS):
    ...
    for i in range(4):
        conv = tf.compat.v1.layers.dense(conv,
                               2**nbits,
                               kernel_regularizer=tf.keras.regularizers.l2(l=0.01),
                               bias_regularizer=tf.keras.regularizers.l2(l=0.01),
                               activation=tf.nn.tanh
                               )
    conv = tf.compat.v1.layers.conv2d(conv, m_iq, 1, strides=1, padding='same')
...
def dense_block_rx(inputs, FLAGS, outshape=None):
    ...
    out2 = tf.compat.v1.layers.dense(out1,
                          encode_rx,
                          kernel_regularizer=tf.keras.regularizers.l2(l=0.01),
                          bias_regularizer=tf.keras.regularizers.l2(l=0.01),
                          activation=activation_fn# tf.nn.tanh
...
def equalizer_block(inputs, FLAGS, ofdmobj):
    ...
    # else:
    #     Kin = n_sc
    chest = tf.compat.v1.layers.dense(chest,
                            K * m_iq,
                            kernel_regularizer=tf.keras.regularizers.l2(l=0.01),
                            bias_regularizer=tf.keras.regularizers.l2(l=0.01),
                            #activation=tf.nn.tanh
    ...
    chest = tf.transpose(a=chest, perm=[0, 1, 3, 2, 4])
    ...
    for it in range(1):
        ...
        chest0 = tf.compat.v1.layers.dense(chest,
                                 layer_size,
                                 kernel_regularizer=tf.keras.regularizers.l2(l=0.01),
                                 bias_regularizer=tf.keras.regularizers.l2(l=0.01),
                                 activation=tf.nn.tanh
                                 )
        chest1 = tf.compat.v1.layers.dense(chest-chest0,
                                 layer_size,
                                 kernel_regularizer=tf.keras.regularizers.l2(l=0.01),
                                 bias_regularizer=tf.keras.regularizers.l2(l=0.01),
                                 activation=tf.nn.tanh
                                 )
        chest2 = tf.compat.v1.layers.dense(chest0-chest1,
                                 layer_size,
                                 kernel_regularizer=tf.keras.regularizers.l2(l=0.01),
                                 bias_regularizer=tf.keras.regularizers.l2(l=0.01),
                                 activation=tf.nn.tanh
                                 )
        chest3 = tf.compat.v1.layers.dense(chest1-chest2,
                                 layer_size,
                                 kernel_regularizer=tf.keras.regularizers.l2(l=0.01),
                                 bias_regularizer=tf.keras.regularizers.l2(l=0.01),
                                 activation=tf.nn.tanh
                                 )
        chest4 = tf.compat.v1.layers.dense(chest2-chest3,
                                 layer_size,
                                 kernel_regularizer=tf.keras.regularizers.l2(l=0.01),
                                 bias_regularizer=tf.keras.regularizers.l2(l=0.01),
                                 activation=tf.nn.tanh
                                 )
        chest5 = tf.compat.v1.layers.dense(chest3-chest4,
                                 layer_size,
                                 kernel_regularizer=tf.keras.regularizers.l2(l=0.01),
                                 bias_regularizer=tf.keras.regularizers.l2(l=0.01),
                                 activation=tf.nn.tanh
        ...
        chest_conj = tf.math.conj(chest)
        chest_square = tf.square(chest)
        equalized_complex = tf.multiply(inputs_complex,chest_conj)
        equalized_complex = tf.compat.v1.div(equalized_complex,chest_square)
        equalized_freq = tf.concat([tf.math.real(equalized_complex),tf.math.imag(equalized_complex)], axis=3)
        chest = equalized_freq

    equalized_complex = layers_conv2d_complex(equalized_complex, n_sc, (1,K), strides=1, padding='valid')
    equalized_complex = tf.transpose(a=equalized_complex, perm=[0, 1, 3, 2])
    equalized_complex = tf.reshape(equalized_complex, [-1,n_sym, n_sc, 1])
    equalized = tf.concat([tf.math.real(equalized_complex),tf.math.imag(equalized_complex)], axis=3)
    ...
    signal_pwr, noise_pwr = tf.nn.moments(x=tf.square(tf.abs(freq_cpx)), axes=[1], keepdims=True)
...
def equalizer_ofdm(inputs, FLAGS, ofdmobj):
    ...
    pilot = tf.compat.v1.layers.dense(chest,
                            pilot_size_iq,
                            kernel_regularizer=tf.keras.regularizers.l2(l=0.01),
                            bias_regularizer=tf.keras.regularizers.l2(l=0.01),
                            # activation=tf.nn.tanh
    ...
    chest_conj = tf.concat([tf.compat.v1.div(tf.math.real(chest_conj), chest_abs), tf.compat.v1.div(tf.math.imag(chest_conj), chest_abs)], axis=-1)
    chest_conj = tf.complex(chest_conj[:,:,:,0:1], chest_conj[:,:,:,1:])
    equalized_complex = tf.multiply(inputs_complex, chest_conj)
    equalized_freq = tf.concat([tf.math.real(equalized_complex), tf.math.imag(equalized_complex)], axis=3)

    corr_cpx = tf.multiply(equalized_complex, tf.math.conj(equalized_complex))
    corr_cpx = layers_conv2d_complex(corr_cpx, K, (1,K), strides=1, padding='valid')
    corr_cpx = tf.transpose(a=corr_cpx,perm=[0,1,3,2])
    corr_re = tf.concat([tf.math.real(corr_cpx), tf.math.imag(corr_cpx)], axis=-1)
    ...
    equalized = tf.concat([tf.math.real(equalized_complex), tf.math.imag(equalized_complex)], axis=-1)
    ...
    equalized = tf.compat.v1.layers.dense(equalized,
                                n_sc * m_iq,
                                kernel_regularizer=tf.keras.regularizers.l2(l=0.01),
                                bias_regularizer=tf.keras.regularizers.l2(l=0.01),
                                )
...
def equalizer_noresdl(inputs, FLAGS, ofdmobj):
    ...
    # Directly Use FFT
    equalized_complex = tf.reshape(equalized_complex, [-1, n_sym, K])
    equalized_complex = tf.signal.ifft(equalized_complex)
    equalized_complex = tf.reshape(equalized_complex, [-1, n_sym, K, 1])
    equalized = tf.concat([tf.math.real(equalized_complex), tf.math.imag(equalized_complex)], axis=-1)
...
def ofdm_dense_rx(inputs, FLAGS, ofdmobj, outshape=None):
    ...
    with tf.compat.v1.variable_scope('fft_like') as scope:
    ...
    with tf.compat.v1.variable_scope('demodulation') as scope:
        out = tf.reshape(out, [-1, n_sym * n_sc * m_iq])
        out = tf.compat.v1.layers.dense(out,
                              data_ofdm * m_iq,
                              kernel_regularizer=tf.keras.regularizers.l2(l=0.01),
        ...
        out = tf.compat.v1.layers.conv2d(out, 2**nbits, 1, strides=1, padding='same')
...
def ofdm_equalized_rx(inputs, FLAGS, ofdmobj, outshape=None):
    ...
    y_cp = tf.compat.v1.layers.dense(y_cp,
                            n_sc * m_iq,
                            kernel_regularizer=tf.keras.regularizers.l2(l=0.01),
                            bias_regularizer=tf.keras.regularizers.l2(l=0.01),
                            # activation=tf.nn.tanh
    ...
    with tf.compat.v1.variable_scope('channel_estimation') as scope:
        # Frequency Domain Inputs IQ data
        inputs_complex = tf.complex(chest[:,:,:,:,0],chest[:,:,:,:,1])
        # inputs_complex = tf.reshape(inputs_complex, [-1, n_sym, K, 1])
        ...
        chest = tf.compat.v1.layers.dense(pilot,
                                n_sym * n_sc * m_iq,
                                kernel_regularizer=tf.keras.regularizers.l2(l=0.01),
                                bias_regularizer=tf.keras.regularizers.l2(l=0.01),
                                #activation=tf.nn.tanh
...
def equalizer_dnn(inputs, FLAGS, ofdmobj):
    ...
    iq_freq = tf.compat.v1.layers.dense(iq_freq,
                            frame_size * 2 - 30,
                            kernel_regularizer=tf.keras.regularizers.l2(l=0.01),
                            bias_regularizer=tf.keras.regularizers.l2(l=0.01),
                            # activation=activation_fn
    ...
    iq_time = tf.compat.v1.layers.dense(iq_time,
                                n_sc * m_iq,
                                kernel_regularizer=tf.keras.regularizers.l2(l=0.01),
                                bias_regularizer=tf.keras.regularizers.l2(l=0.01),
                                )
